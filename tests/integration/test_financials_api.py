...
@pytest.mark.asyncio
async def test_get_quarter_metric_api(
    db_session: AsyncSession,
) -> None:
    company_repository = CompanyRepository(db_session)
    security_repository = SecurityRepository(db_session)
    ...
    company = await company_repository.add(
        Company(
            cvm_code="API901",
            cnpj="92000000000102",
            legal_name="EMPRESA METRIC API S.A.",
            trade_name="METRIC API",
        )
    )
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                "/financials/by-ticker/FMET3/quarterly/2026/2/metrics/revenue"
            )
    ...
    assert body["ticker"] == "FMET3"
    assert body["year"] == 2026
    assert body["quarter"] == 2

    assert body["statement_type"] == "DRE"
...
@pytest.mark.asyncio
async def test_unknown_metric_returns_400(
    db_session: AsyncSession,
) -> None:
    def override_financial_query_service() -> FinancialQueryService:
        return FinancialQueryService(db_session)
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                "/financials/by-ticker/FMET3/quarterly/2026/2/metrics/unknown"
            )
    ...
    assert response.json() == {"detail": ("Unknown financial metric: unknown")}
